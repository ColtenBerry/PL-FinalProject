...
--   runInputT quiltSettings . flip evalStateT (QS Nothing Nothing defaultSize) $ loop
--  where
--   loop :: QuiltM ()
--   loop = do
...
-- sizeCmd [] = do
--   cur <- gets quiltSize
--   lift $ outputStrLn $ "The current size is " ++ showSize cur ++ "."
-- sizeCmd (sz:_) =
--   if all isDigit sz
--     then do
--       let n = read sz
--       modify (\qs -> qs { quiltSize = n })
--       lift $ outputStrLn $ "Size for saved images is now " ++ showSize n ++ "."
--     else
--       lift $ outputStrLn "Please provide an integer size."

-- saveCmd :: [String] -> QuiltM ()
-- saveCmd []     = lift $ outputStrLn "Please provide a file name to save."
-- saveCmd (fn:_) = do
--   sz <- gets quiltSize
--   saveQFun True sz fn
...
--         lift $ maybe (return ()) (const $ outputStrLn $ "source -> " ++ srcFn) ms

-- -- Helpers to add/remove extensions
...
--   let toCoord r c = q
--         (2 * fromIntegral r / fromIntegral qSize - 1)
--         (-(2 * fromIntegral c / fromIntegral qSize - 1))
--       img = ImageRGB8 $ generateImage (\r c -> toPixel $ toCoord r c) qSize qSize
--   savePngImage fn img
...
--   Right fun -> do
--     modify (\qs -> qs { quiltFun = Just fun, quiltSrc = Just s })
--     saveQFun False defaultSize "quilt"
